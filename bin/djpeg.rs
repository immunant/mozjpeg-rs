 ...
 */ {
    ...
    while argn < argc {
        ...
        /* else done parsing switches */
            ...
            if keymatch(arg, b"bmp\x00".as_ptr() as *const c_char, 1i32) != 0 {
                /* BMP output format. */
                requested_fmt = FMT_BMP
            } else if keymatch(arg, b"colors\x00".as_ptr() as *const c_char, 1i32) != 0
                || keymatch(arg, b"colours\x00".as_ptr() as *const c_char, 1i32) != 0
                || keymatch(arg, b"quantize\x00".as_ptr() as *const c_char, 1i32) != 0
                || keymatch(arg, b"quantise\x00".as_ptr() as *const c_char, 1i32) != 0
            {
                let mut val: c_int = 0;
                argn += 1;
                if argn >= argc {
            ...
            } else if keymatch(arg, b"dither\x00".as_ptr() as *const c_char, 2i32) != 0 {
                ...
                || keymatch(arg, b"verbose\x00".as_ptr() as *const c_char, 1i32) != 0
            {
                ...
                if keymatch(arg, b"fast\x00".as_ptr() as *const c_char, 1i32) != 0 {
                ...
                } else if keymatch(arg, b"gif\x00".as_ptr() as *const c_char, 1i32) != 0 {
                    /* GIF output format. */
                    ...
                    || keymatch(arg, b"greyscale\x00".as_ptr() as *const c_char, 2i32) != 0
                {
                ...
                } else if keymatch(arg, b"rgb\x00".as_ptr() as *const c_char, 2i32) != 0 {
                    /* Force RGB output. */
                    (*cinfo).out_color_space = JCS_RGB
                } else if keymatch(arg, b"rgb565\x00".as_ptr() as *const c_char, 2i32) != 0 {
                    /* Force RGB565 output. */
                    ...
                    jpeg_save_markers(cinfo, JPEG_APP0 + 2i32, 0xffffu32);
                } else if keymatch(arg, b"map\x00".as_ptr() as *const c_char, 3i32) != 0 {
                    ...
                    if for_real != 0 {
                        let mut mapfile: *mut FILE =
                            fopen(*argv.offset(argn as isize), READ_BINARY.as_ptr());
                        if mapfile.is_null() {
                            fprintf(
                                stderr,
                                b"%s: can\'t open %s\n\x00".as_ptr() as *const c_char,
                ...
                } else if keymatch(arg, b"nosmooth\x00".as_ptr() as *const c_char, 3i32) != 0 {
                    /* Suppress fancy upsampling */
                    (*cinfo).do_fancy_upsampling = FALSE
                } else if keymatch(arg, b"onepass\x00".as_ptr() as *const c_char, 3i32) != 0 {
                    /* Use fast one-pass quantization. */
                    (*cinfo).two_pass_quantize = FALSE
                } else if keymatch(arg, b"os2\x00".as_ptr() as *const c_char, 3i32) != 0 {
                ...
                } else if keymatch(arg, b"memsrc\x00".as_ptr() as *const c_char, 2i32) != 0 {
                    /* Use in-memory source manager */
                    memsrc = TRUE
                } else if keymatch(arg, b"pnm\x00".as_ptr() as *const c_char, 1i32) != 0
                    || keymatch(arg, b"ppm\x00".as_ptr() as *const c_char, 1i32) != 0
                {
                    /* PPM/PGM output format. */
                    requested_fmt = FMT_PPM
                } else if keymatch(arg, b"rle\x00".as_ptr() as *const c_char, 1i32) != 0 {
                    /* RLE output format. */
                    requested_fmt = FMT_RLE
                } else if keymatch(arg, b"scale\x00".as_ptr() as *const c_char, 2i32) != 0 {
                ...
                } else if keymatch(arg, b"skip\x00".as_ptr() as *const c_char, 2i32) != 0 {
                    argn += 1;
                    if argn >= argc {
                        usage();
                    }
                    ...
                    skip = TRUE
                } else if keymatch(arg, b"crop\x00".as_ptr() as *const c_char, 2i32) != 0 {
                    let mut c: c_char = 0;
                    argn += 1;
                    if argn >= argc {
                        usage();
    ...
    loop {
        ...
        if traceit != 0 {
            ...
            if ch == '\r' as c_uint {
                ...
                fprintf(stderr, b"\\\\\x00".as_ptr() as *const c_char);
                ...
                != 0
            {
                ...
                fprintf(stderr, b"\\%03o\x00".as_ptr() as *const c_char, ch);
            }
            lastch = ch
        }
    }
...
unsafe fn main_0(mut argc: c_int, mut argv: *mut *mut c_char) -> c_int {
    ...
    let mut output_file: *mut FILE = ::std::ptr::null_mut::<FILE>();
    ...
    let mut cinfo: jpeg_decompress_struct = jpeg_decompress_struct {
        ...
        Adobe_transform: 0,
        CCIR601_sampling: 0,
        marker_list: ::std::ptr::null_mut::<jpeg_marker_struct>(),
        max_h_samp_factor: 0,
        max_v_samp_factor: 0,
        ...
        inputctl: ::std::ptr::null_mut::<jpeg_input_controller>(),
        marker: ::std::ptr::null_mut::<jpeg_marker_reader>(),
        entropy: ::std::ptr::null_mut::<jpeg_entropy_decoder>(),
        idct: ::std::ptr::null_mut::<jpeg_inverse_dct>(),
        upsample: ::std::ptr::null_mut::<jpeg_upsampler>(),
        cconvert: ::std::ptr::null_mut::<jpeg_color_deconverter>(),
        cquantize: ::std::ptr::null_mut::<jpeg_color_quantizer>(),
    };
    ...
    if memsrc != 0 {
        ...
        loop {
            inbuffer = realloc(inbuffer as *mut c_void, insize + INPUT_BUF_SIZE as c_ulong)
                as *mut c_uchar;
            if inbuffer.is_null() {
                fprintf(
                    stderr,
    ...
    if skip != 0 {
        ...
        cinfo.output_height = cinfo.output_height - (skip_end - skip_start + 1u32);
        ...
        jpeg_skip_scanlines(&mut cinfo, skip_end - skip_start + 1u32);
        ...
        jpeg_skip_scanlines(&mut cinfo, cinfo.output_height - crop_y - crop_height);
    } else {
    ...
    if !icc_filename.is_null() {
        let mut icc_profile: *mut JOCTET = ::std::ptr::null_mut::<JOCTET>();
        let mut icc_len: c_uint = 0;
        let mut icc_file: *mut FILE = fopen(icc_filename, WRITE_BINARY.as_ptr());
        if icc_file.is_null() {
            fprintf(
                stderr,
                b"%s: can\'t open %s\n\x00".as_ptr() as *const c_char,
