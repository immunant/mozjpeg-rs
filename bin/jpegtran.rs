...
static mut transformoption: jpeg_transform_info = jpeg_transform_info {
    ...
    num_components: 0,
    workspace_coef_arrays: ::std::ptr::null::<jvirt_barray_ptr>() as *mut jvirt_barray_ptr,
    output_width: 0,
    output_height: 0,
    x_crop_offset: 0,
    y_crop_offset: 0,
    ...
    while argn < argc {
        ...
        /* else done parsing switches */
            ...
            if keymatch(arg, b"arithmetic\x00".as_ptr() as *const c_char, 1i32) != 0 {
                ...
                if keymatch(arg, b"copy\x00".as_ptr() as *const c_char, 2i32) != 0 {
                    ...
                    || keymatch(arg, b"verbose\x00".as_ptr() as *const c_char, 1i32) != 0
                {
                    ...
                    if keymatch(arg, b"flip\x00".as_ptr() as *const c_char, 1i32) != 0 {
                        ...
                        prefer_smallest = FALSE
                    } else if keymatch(arg, b"fastcrush\x00".as_ptr() as *const c_char, 4i32) != 0 {
                        jpeg_c_set_bool_param(cinfo, JBOOLEAN_OPTIMIZE_SCANS, FALSE);
                    } else if keymatch(arg, b"grayscale\x00".as_ptr() as *const c_char, 1i32) != 0
                        || keymatch(arg, b"greyscale\x00".as_ptr() as *const c_char, 1i32) != 0
                    {
                        /* Force to grayscale. */
                    ...
                    } else if keymatch(arg, b"perfect\x00".as_ptr() as *const c_char, 2i32) != 0 {
                        /* Fail if there is any partial edge MCUs that the transform can't
                         * handle. */
                        transformoption.perfect = TRUE
                    } else if keymatch(arg, b"progressive\x00".as_ptr() as *const c_char, 2i32) != 0
                    {
                    ...
                    /* We must postpone execution until num_components is known. */
                        ...
                        prefer_smallest = FALSE
                    } else if keymatch(arg, b"rotate\x00".as_ptr() as *const c_char, 2i32) != 0 {
                        ...
                        prefer_smallest = FALSE
                    } else if keymatch(arg, b"scans\x00".as_ptr() as *const c_char, 1i32) != 0 {
                        /* Set scan script. */
                        argn += 1;
                        if argn >= argc {
                            /* advance to next argument */
                    ...
                    } else if keymatch(arg, b"transpose\x00".as_ptr() as *const c_char, 1i32) != 0 {
                        /* Transpose (across UL-to-LR axis). */
                        select_transform(JXFORM_TRANSPOSE);
                        prefer_smallest = FALSE
                    } else if keymatch(arg, b"transverse\x00".as_ptr() as *const c_char, 6i32) != 0
                    {
                        /* Transverse transpose (across UR-to-LL axis). */
                        select_transform(JXFORM_TRANSVERSE);
                        prefer_smallest = FALSE
                    } else if keymatch(arg, b"trim\x00".as_ptr() as *const c_char, 3i32) != 0 {
...
unsafe fn main_0(mut argc: c_int, mut argv: *mut *mut c_char) -> c_int {
     ...
     */
    let mut file_index: c_int = parse_switches(&mut dstinfo, argc, argv, 0i32, FALSE);
    ...
    let mut src_coef_arrays: *mut jvirt_barray_ptr = jpeg_read_coefficients(&mut srcinfo);
    /* Initialize destination compression parameters from source values */
    jpeg_copy_critical_parameters(&mut srcinfo, &mut dstinfo);
    /* Adjust destination parameters if required by transform options;
     * also find out which set of coefficient arrays will hold the output.
     */
    let mut dst_coef_arrays: *mut jvirt_barray_ptr = jtransform_adjust_parameters(
        &mut srcinfo,
        &mut dstinfo,
        src_coef_arrays,
        &mut transformoption,
        ...
        && jpeg_c_get_int_param(&mut dstinfo, JINT_COMPRESS_PROFILE) == JCP_MAX_COMPRESSION as c_int
    {
        ...
        let mut nbytes_0: size_t = fwrite(buffer as *const c_void, 1u64, size, fp);
