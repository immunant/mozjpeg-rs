 ...
 */ {
    ...
    let mut force_baseline: boolean = FALSE;
    let mut simple_progressive: boolean = if (*cinfo).num_scans == 0i32 {
        FALSE
    } else {
        TRUE
    };
    ...
    while argn < argc {
        ...
        /* else done parsing switches */
            ...
            if keymatch(arg, b"arithmetic\x00".as_ptr() as *const c_char, 1i32) != 0 {
                ...
                if keymatch(arg, b"baseline\x00".as_ptr() as *const c_char, 1i32) != 0 {
                ...
                } else if keymatch(arg, b"dct\x00".as_ptr() as *const c_char, 2i32) != 0 {
                ...
                } else if keymatch(arg, b"debug\x00".as_ptr() as *const c_char, 1i32) != 0
                    || keymatch(arg, b"verbose\x00".as_ptr() as *const c_char, 1i32) != 0
                {
                ...
                } else if keymatch(arg, b"version\x00".as_ptr() as *const c_char, 4i32) != 0 {
                    fprintf(
                        stderr,
                        b"%s version %s (build %s)\n\x00".as_ptr() as *const c_char,
                        PACKAGE_NAME.as_ptr(),
                    ...
                    if keymatch(arg, b"fastcrush\x00".as_ptr() as *const c_char, 4i32) != 0 {
                        jpeg_c_set_bool_param(cinfo, JBOOLEAN_OPTIMIZE_SCANS, FALSE);
                    } else if keymatch(arg, b"grayscale\x00".as_ptr() as *const c_char, 2i32) != 0
                        || keymatch(arg, b"greyscale\x00".as_ptr() as *const c_char, 2i32) != 0
                    {
                        /* Force a monochrome JPEG file to be generated. */
                        jpeg_set_colorspace(cinfo, JCS_GRAYSCALE);
                    } else if keymatch(arg, b"rgb\x00".as_ptr() as *const c_char, 3i32) != 0 {
                        /* Force an RGB JPEG file to be generated. */
                        jpeg_set_colorspace(cinfo, JCS_RGB);
                    } else if keymatch(arg, b"lambda1\x00".as_ptr() as *const c_char, 7i32) != 0 {
                        argn += 1;
                        if argn >= argc {
                            /* advance to next argument */
                            usage();
                    ...
                    } else if keymatch(arg, b"lambda2\x00".as_ptr() as *const c_char, 7i32) != 0 {
                        argn += 1;
                        if argn >= argc {
                            /* advance to next argument */
                            usage();
                    ...
                    } else if keymatch(arg, b"icc\x00".as_ptr() as *const c_char, 1i32) != 0 {
                        /* Set ICC filename. */
                        argn += 1;
                        if argn >= argc {
                            /* advance to next argument */
                    ...
                    } else if keymatch(arg, b"maxmemory\x00".as_ptr() as *const c_char, 3i32) != 0 {
                    ...
                    } else if keymatch(arg, b"dc-scan-opt\x00".as_ptr() as *const c_char, 3i32) != 0
                    {
                        argn += 1;
                        if argn >= argc {
                            /* advance to next argument */
                    ...
                    } else if keymatch(arg, b"optimize\x00".as_ptr() as *const c_char, 1i32) != 0
                        || keymatch(arg, b"optimise\x00".as_ptr() as *const c_char, 1i32) != 0
                    {
                        /* Enable entropy parm optimization. */
                        (*cinfo).optimize_coding = TRUE
                    } else if keymatch(arg, b"outfile\x00".as_ptr() as *const c_char, 4i32) != 0 {
                    ...
                    } else if keymatch(arg, b"progressive\x00".as_ptr() as *const c_char, 1i32) != 0
                    {
                        /* Select simple progressive mode. */
                        simple_progressive = TRUE
                    /* We must postpone execution until num_components is known. */
                    } else if keymatch(arg, b"memdst\x00".as_ptr() as *const c_char, 2i32) != 0 {
                        /* Use in-memory destination manager */
                        memdst = TRUE
                    } else if keymatch(arg, b"quality\x00".as_ptr() as *const c_char, 1i32) != 0 {
                    ...
                    } else if keymatch(arg, b"qslots\x00".as_ptr() as *const c_char, 2i32) != 0 {
                     ...
                     */
                    } else if keymatch(arg, b"qtables\x00".as_ptr() as *const c_char, 2i32) != 0 {
                    ...
                    } else if keymatch(arg, b"quant-table\x00".as_ptr() as *const c_char, 7i32) != 0
                    {
                        argn += 1;
                        if argn >= argc {
                            /* advance to next argument */
                    ...
                    } else if keymatch(arg, b"quant-baseline\x00".as_ptr() as *const c_char, 7i32)
                        != 0
                    {
                        /* Force quantization table to meet baseline requirements */
                        force_baseline = TRUE
                    } else if keymatch(arg, b"restart\x00".as_ptr() as *const c_char, 1i32) != 0 {
                        let mut lval_0: c_long = 0;
                        let mut ch_0: c_char = 'x' as c_char;
                        argn += 1;
                        if argn >= argc {
                            /* advance to next argument */
                            usage();
                    ...
                    } else if keymatch(arg, b"revert\x00".as_ptr() as *const c_char, 3i32) != 0 {
                        /* revert to old JPEG default */
                        jpeg_c_set_int_param(cinfo, JINT_COMPRESS_PROFILE, JCP_FASTEST as c_int);
                        jpeg_set_defaults(cinfo);
                    } else if keymatch(arg, b"sample\x00".as_ptr() as *const c_char, 2i32) != 0 {
                     ...
                     */
                    } else if keymatch(arg, b"scans\x00".as_ptr() as *const c_char, 4i32) != 0 {
                        /* Set scan script. */
                        argn += 1;
                        if argn >= argc {
                            /* advance to next argument */
                    ...
                    } else if keymatch(arg, b"smooth\x00".as_ptr() as *const c_char, 2i32) != 0 {
                    ...
                    } else if keymatch(arg, b"targa\x00".as_ptr() as *const c_char, 1i32) != 0 {
                        /* Input file is Targa format. */
                        is_targa = TRUE
                    } else if keymatch(arg, b"notrellis-dc\x00".as_ptr() as *const c_char, 11i32)
                        != 0
                    {
                        /* disable trellis quantization */
                        jpeg_c_set_bool_param(cinfo, JBOOLEAN_TRELLIS_QUANT_DC, FALSE);
                    } else if keymatch(arg, b"notrellis\x00".as_ptr() as *const c_char, 1i32) != 0 {
                        /* disable trellis quantization */
                    ...
                    } else if keymatch(arg, b"trellis-dc\x00".as_ptr() as *const c_char, 9i32) != 0
                    {
                        /* enable DC trellis quantization */
                        jpeg_c_set_bool_param(cinfo, JBOOLEAN_TRELLIS_QUANT_DC, TRUE);
                    } else if keymatch(arg, b"tune-psnr\x00".as_ptr() as *const c_char, 6i32) != 0 {
                        jpeg_c_set_int_param(cinfo, JINT_BASE_QUANT_TBL_IDX, 1i32);
                        jpeg_c_set_float_param(cinfo, JFLOAT_LAMBDA_LOG_SCALE1, 9f32);
                        jpeg_c_set_float_param(cinfo, JFLOAT_LAMBDA_LOG_SCALE2, 0f32);
                        jpeg_c_set_bool_param(cinfo, JBOOLEAN_USE_LAMBDA_WEIGHT_TBL, FALSE);
                        jpeg_set_quality(cinfo, 75i32, TRUE);
                    } else if keymatch(arg, b"tune-ssim\x00".as_ptr() as *const c_char, 6i32) != 0 {
                        jpeg_c_set_int_param(cinfo, JINT_BASE_QUANT_TBL_IDX, 1i32);
                        jpeg_c_set_float_param(cinfo, JFLOAT_LAMBDA_LOG_SCALE1, 11.5f32);
                        jpeg_c_set_float_param(cinfo, JFLOAT_LAMBDA_LOG_SCALE2, 12.75f32);
                        jpeg_c_set_bool_param(cinfo, JBOOLEAN_USE_LAMBDA_WEIGHT_TBL, FALSE);
                        jpeg_set_quality(cinfo, 75i32, TRUE);
                    } else if keymatch(arg, b"tune-ms-ssim\x00".as_ptr() as *const c_char, 6i32)
                        != 0
                    {
                        jpeg_c_set_int_param(cinfo, JINT_BASE_QUANT_TBL_IDX, 3i32);
                        jpeg_c_set_float_param(cinfo, JFLOAT_LAMBDA_LOG_SCALE1, 12f32);
                        jpeg_c_set_float_param(cinfo, JFLOAT_LAMBDA_LOG_SCALE2, 13f32);
                        jpeg_c_set_bool_param(cinfo, JBOOLEAN_USE_LAMBDA_WEIGHT_TBL, TRUE);
                        jpeg_set_quality(cinfo, 75i32, TRUE);
                    } else if keymatch(arg, b"tune-hvs-psnr\x00".as_ptr() as *const c_char, 6i32)
                        != 0
                    {
                        jpeg_c_set_int_param(cinfo, JINT_BASE_QUANT_TBL_IDX, 3i32);
                        jpeg_c_set_float_param(cinfo, JFLOAT_LAMBDA_LOG_SCALE1, 14.75f32);
                        jpeg_c_set_float_param(cinfo, JFLOAT_LAMBDA_LOG_SCALE2, 16.5f32);
                        jpeg_c_set_bool_param(cinfo, JBOOLEAN_USE_LAMBDA_WEIGHT_TBL, TRUE);
                        jpeg_set_quality(cinfo, 75i32, TRUE);
                    } else if keymatch(arg, b"noovershoot\x00".as_ptr() as *const c_char, 11i32)
                        != 0
                    {
                        jpeg_c_set_bool_param(cinfo, JBOOLEAN_OVERSHOOT_DERINGING, FALSE);
                    } else if keymatch(arg, b"nojfif\x00".as_ptr() as *const c_char, 6i32) != 0 {
...
unsafe fn main_0(mut argc: c_int, mut argv: *mut *mut c_char) -> c_int {
    let mut input_file: *mut FILE = ::std::ptr::null_mut::<FILE>();
    let mut icc_len: c_long = 0i64;
    let mut outsize: c_ulong = 0u64;
    let mut cinfo: jpeg_compress_struct = jpeg_compress_struct {
        err: ::std::ptr::null_mut::<jpeg_error_mgr>(),
        ...
        quant_tbl_ptrs: [::std::ptr::null_mut::<JQUANT_TBL>(); 4],
        dc_huff_tbl_ptrs: [::std::ptr::null_mut::<JHUFF_TBL>(); 4],
        ac_huff_tbl_ptrs: [::std::ptr::null_mut::<JHUFF_TBL>(); 4],
        arith_dc_L: [0; 16],
        arith_dc_U: [0; 16],
        arith_ac_K: [0; 16],
        num_scans: 0,
        ...
        total_iMCU_rows: 0,
        comps_in_scan: 0,
        cur_comp_info: [::std::ptr::null_mut::<jpeg_component_info>(); 4],
        MCUs_per_row: 0,
        MCU_rows_in_scan: 0,
        blocks_in_MCU: 0,
        MCU_membership: [0; 10],
     ...
     * the input file.
     */
    let mut file_index: c_int = parse_switches(&mut cinfo, argc, argv, 0i32, FALSE);
    ...
    if !icc_filename.is_null() {
        let mut icc_file: *mut FILE = fopen(icc_filename, READ_BINARY.as_ptr());
        if icc_file.is_null() {
            fprintf(
                stderr,
                b"%s: can\'t open %s\n\x00".as_ptr() as *const c_char,
    ...
    let mut src_mgr: cjpeg_source_ptr = select_file_type(&mut cinfo, input_file);
    ...
    if copy_markers != 0 {
         ...
         */
        let mut marker: jpeg_saved_marker_ptr = (*src_mgr).marker_list; /* reject duplicate JFIF */
