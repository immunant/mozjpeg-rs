use libc::{c_uint, c_double, c_char, c_long, c_ulong, c_void, c_int};extern "C" {
    #[no_mangle]
...
use libc;use super::simd::x86_64::jsimd::{jsimd_can_huff_encode_one_block,
                           ...
                           jpeg_component_info, jpeg_destination_mgr,
                           ...
                           DCTSIZE2, JBLOCKARRAY, JCOEFPTR, JCS_CMYK,
                           ...
                           JDCT_ISLOW, JPOOL_IMAGE, JSAMPARRAY, JSAMPIMAGE,
                           JSAMPROW, J_COLOR_SPACE, J_DCT_METHOD,
                           NUM_HUFF_TBLS};pub use crate::jmorecfg_h::{boolean, JCOEF, JDIMENSION, FALSE, JOCTET,
                            JSAMPLE, TRUE, UINT16, UINT8};pub use crate::internal::__CHAR_BIT__;pub use crate::limits_h::CHAR_BIT;pub use crate::jpegint_h::{jpeg_natural_order, JBUF_CRANK_DEST,
                           JBUF_PASS_THRU, JBUF_REQUANT, JBUF_SAVE_AND_PASS,
                           JBUF_SAVE_SOURCE, JLONG, J_BUF_MODE};pub use super::jerror::{C2RustUnnamed_3, JERR_ARITH_NOTIMPL,
...
unsafe extern "C" fn start_pass_huff(
    ...
    while ci < (*cinfo).comps_in_scan {
     ...
     (*cinfo).cur_comp_info[ci as usize]; let mut dctbl:   c_int =  (*compptr).dc_tbl_no; let mut actbl:   c_int =  (*compptr).ac_tbl_no;
        if gather_statistics != 0 {
...
pub unsafe extern "C" fn jpeg_make_c_derived_tbl(
    ...
    if (*pdtbl).is_null() {
     ...
     let mut dtbl:   *mut c_derived_tbl =  *pdtbl; let mut p:   c_int =  0i32; let mut l:   c_int =  1i32;
    while l <= 16i32 {
    ...
    huffsize[p as usize] = 0i8;
     ...
     let mut lastp:   c_int =  p; let mut code:   c_uint =  0u32; let mut si:   c_int =  huffsize[0] as c_int;
    p = 0i32;
...
unsafe extern "C" fn flush_bits(mut state: *mut working_state) -> boolean {
     ...
     let mut _buffer:  [JOCTET; 256] =  [0; 256]; let mut buffer:  *mut JOCTET =
     ::std::ptr::null_mut::< JOCTET>();   let mut localbuf:  c_int =  0i32;
    
     let mut put_buffer:   size_t =  (*state).cur.put_buffer; let mut put_bits:   c_int =  (*state).cur.put_bits;
...
unsafe extern "C" fn encode_one_block(
     ...
     * Agner Fog.
     */
     let mut temp2:   c_int =  *block.offset(0) as c_int - last_dc_val; let mut temp:   c_int =  temp2; let mut temp3:   c_int =
     temp
        >> CHAR_BIT as c_ulong *
    ...
    temp2 += temp3;
     ...
     let mut nbits:   c_int =
     jpeg_nbits_table[temp as usize] as c_int; let mut code:   c_int =  (*dctbl).ehufco[nbits as usize] as c_int; let mut size:   c_int =  (*dctbl).ehufsi[nbits as usize] as c_int;
    if put_bits > 47i32 {
...
unsafe extern "C" fn encode_mcu_huff(
    ...
    }; let mut blkn:  c_int =  0; let mut ci:  c_int =  0; let mut compptr:  *mut jpeg_component_info =
...
unsafe extern "C" fn htest_one_block(
    ...
    *fresh1303 += 1;
     ...
     let mut r:   c_int =  0i32; let mut k:   c_int =  1i32;
    while k < DCTSIZE2 {
        temp = *block.offset(
            *jpeg_natural_order
                .as_ptr()
...
unsafe extern "C" fn encode_mcu_gather(
    ...
    mut MCU_data: *mut JBLOCKROW,
      ...
      let mut ci:  c_int =  0;let mut entropy: huff_entropy_ptr = (*cinfo).entropy as huff_entropy_ptr;
...
pub unsafe extern "C" fn jpeg_gen_optimal_table(
    ...
    mut freq: *mut c_long,
     ...
     let mut bits:  [UINT8; 33] =  [0; 33]; let mut codesize:  [c_int; 257] =  [0; 257]; let mut others:  [c_int; 257] =  [0; 257];   let mut j:  c_int =  0;
    ...
    *freq.offset(256) = 1i64;
    ...
    /* In case of ties, take the larger symbol number */
    {
         ...
         let mut c1:   c_int =  -1i32; let mut v:   c_long =  1000000000i64;
        i = 0i32;
        while i <= 256i32 {
    ...
    while i <= 256i32 {
        if codesize[i as usize] != 0 {
            ...
            if codesize[i as usize] > MAX_CLEN {
                (*(*cinfo).err).msg_code =
                    super::jerror::JERR_HUFF_CLEN_OVERFLOW as c_int;
                Some(
                    (*(*cinfo).err)
                        .error_exit
                        .expect("non-null function pointer"),
    ...
    while i > 16i32 {
        while bits[i as usize] as c_int > 0i32 {
            ...
            bits[(j + 1i32) as usize] =
                (bits[(j + 1i32) as usize] as c_int + 2i32) as UINT8; /* two new symbols in this length */
...
unsafe extern "C" fn finish_pass_gather(mut cinfo: j_compress_ptr) {
      let mut did_dc:  [boolean; 4] =  [0; 4]; let mut did_ac:  [boolean; 4] =  [0; 4];let mut entropy: huff_entropy_ptr = (*cinfo).entropy as huff_entropy_ptr;
