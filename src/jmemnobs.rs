...
use crate::stdlib::{free, malloc};use libc::{c_int, c_ulong, c_void, c_long, self};pub use crate::stdlib::{_IO_codecvt, _IO_lock_t, _IO_marker, _IO_wide_data,
                        __off64_t, __off_t, FILE, _IO_FILE};pub use crate::jmorecfg_h::{boolean, JCOEF, JDIMENSION, JSAMPLE};pub use crate::stddef_h::size_t;pub use crate::jmemsys_h::{backing_store_ptr, backing_store_struct};pub use crate::jpeglib_h::{j_common_ptr, jpeg_common_struct, jpeg_error_mgr,
...
pub unsafe extern "C" fn jpeg_mem_available(
    ...
    if (*(*cinfo).mem).max_memory_to_use != 0 {
        if (*(*cinfo).mem).max_memory_to_use as size_t > already_allocated {
            return (*(*cinfo).mem).max_memory_to_use as c_ulong - already_allocated;
        } else {
            return 0u64;
        }
    } else {
