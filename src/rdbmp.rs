...
/* Read the colormap from a BMP file */
{
    ...
    if (*(*sinfo).cinfo).in_color_space == JCS_UNKNOWN && gray != 0 {
        (*(*sinfo).cinfo).in_color_space = JCS_GRAYSCALE
    }
    if (*(*sinfo).cinfo).in_color_space == JCS_GRAYSCALE && gray == 0 {
...
/* This version is for reading 8-bit colormap indexes */ {
    let mut t: c_int = 0;
    let mut inptr: JSAMPROW = ::std::ptr::null_mut::<JSAMPLE>();
    ...
    let mut outptr: JSAMPROW = *(*source).pub_0.buffer.offset(0);
...
unsafe extern "C" fn start_input_bmp(
    ...
    let mut biClrUsed: c_uint = 0u32;
    let mut mapentrysize: c_int = 0i32;
    let mut row_width: JDIMENSION = 0u32;
    ...
    if bmpfileheader[0] as c_int + ((bmpfileheader[(0i32 + 1i32) as usize] as c_int) << 8i32)
        != 0x4d42i32
    {
        /* 'BM' */
    ...
    let mut bfOffBits: c_uint = bmpfileheader[10] as c_uint
    ...
    let mut headerSize: c_uint = bmpinfoheader[0] as c_uint
    ...
    match headerSize {
        12 => {
            ...
            biWidth = bmpinfoheader[4] as c_int
                + ((bmpinfoheader[(4i32 + 1i32) as usize] as c_int) << 8i32); /* OS/2 uses RGBTRIPLE colormap */
            biHeight = bmpinfoheader[6] as c_int
                + ((bmpinfoheader[(6i32 + 1i32) as usize] as c_int) << 8i32);
            biPlanes = (bmpinfoheader[8] as c_int
                + ((bmpinfoheader[(8i32 + 1i32) as usize] as c_int) << 8i32))
                as c_ushort;
            (*source).bits_per_pixel = bmpinfoheader[10] as c_int
                + ((bmpinfoheader[(10i32 + 1i32) as usize] as c_int) << 8i32);
            match (*source).bits_per_pixel {
                8 => {
                    /* colormapped image */
                    mapentrysize = 3i32;
        ...
        40 | 64 => {
            biWidth = (bmpinfoheader[4] as c_uint
                ...
                + ((bmpinfoheader[(4i32 + 3i32) as usize] as c_uint) << 24i32))
                as c_int;
            biHeight = (bmpinfoheader[8] as c_uint
                ...
                + ((bmpinfoheader[(8i32 + 3i32) as usize] as c_uint) << 24i32))
                as c_int;
            biPlanes = (bmpinfoheader[12] as c_int
                + ((bmpinfoheader[(12i32 + 1i32) as usize] as c_int) << 8i32))
                as c_ushort;
            (*source).bits_per_pixel = bmpinfoheader[14] as c_int
                + ((bmpinfoheader[(14i32 + 1i32) as usize] as c_int) << 8i32);

            let mut biCompression: c_uint = bmpinfoheader[16] as c_uint
                + ((bmpinfoheader[(16i32 + 1i32) as usize] as c_uint) << 8i32)
                + ((bmpinfoheader[(16i32 + 2i32) as usize] as c_uint) << 16i32)
                + ((bmpinfoheader[(16i32 + 3i32) as usize] as c_uint) << 24i32);
            let mut biXPelsPerMeter: c_int = (bmpinfoheader[24] as c_uint
                + ((bmpinfoheader[(24i32 + 1i32) as usize] as c_uint) << 8i32)
                + ((bmpinfoheader[(24i32 + 2i32) as usize] as c_uint) << 16i32)
                + ((bmpinfoheader[(24i32 + 3i32) as usize] as c_uint) << 24i32))
                as c_int;
            let mut biYPelsPerMeter: c_int = (bmpinfoheader[28] as c_uint
                + ((bmpinfoheader[(28i32 + 1i32) as usize] as c_uint) << 8i32)
                + ((bmpinfoheader[(28i32 + 2i32) as usize] as c_uint) << 16i32)
                + ((bmpinfoheader[(28i32 + 3i32) as usize] as c_uint) << 24i32))
                as c_int;
            biClrUsed = bmpinfoheader[32] as c_uint
                + ((bmpinfoheader[(32i32 + 1i32) as usize] as c_uint) << 8i32)
                + ((bmpinfoheader[(32i32 + 2i32) as usize] as c_uint) << 16i32)
                + ((bmpinfoheader[(32i32 + 3i32) as usize] as c_uint) << 24i32);
    ...
    if mapentrysize > 0i32 {
        ...
        .expect("non-null function pointer")(
            cinfo as j_common_ptr, JPOOL_IMAGE, biClrUsed, 3u32
        );
    ...
    if biWidth as c_ulonglong * (*cinfo).input_components as c_ulonglong > 0xffffffffu64 {
...
pub unsafe extern "C" fn jinit_read_bmp(
    ...
    let mut source: bmp_source_ptr = Some(
        (*(*cinfo).mem)
            .alloc_small
            .expect("non-null function pointer"),
    )
