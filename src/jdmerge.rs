pub use crate::jdmrg565_c::{
    ...
    h2v2_merged_upsample_565_be, h2v2_merged_upsample_565_le,
};
pub use crate::jdmrgext_c::{
    extbgr_h2v1_merged_upsample_internal, extbgr_h2v2_merged_upsample_internal,
    extbgrx_h2v1_merged_upsample_internal, extbgrx_h2v2_merged_upsample_internal,
    extrgb_h2v1_merged_upsample_internal, extrgb_h2v2_merged_upsample_internal,
    extrgbx_h2v1_merged_upsample_internal, extrgbx_h2v2_merged_upsample_internal,
    extxbgr_h2v1_merged_upsample_internal, extxbgr_h2v2_merged_upsample_internal,
    extxrgb_h2v1_merged_upsample_internal, extxrgb_h2v2_merged_upsample_internal,
    h2v1_merged_upsample_internal, h2v2_merged_upsample_internal,
};
pub use crate::jmorecfg_h::{
    boolean, CENTERJSAMPLE, EXT_BGRX_BLUE, EXT_BGRX_GREEN, EXT_BGRX_PIXELSIZE, EXT_BGRX_RED,
    EXT_BGR_BLUE, EXT_BGR_GREEN, EXT_BGR_PIXELSIZE, EXT_BGR_RED, EXT_RGBX_BLUE, EXT_RGBX_GREEN,
    EXT_RGBX_PIXELSIZE, EXT_RGBX_RED, EXT_RGB_BLUE, EXT_RGB_GREEN, EXT_RGB_PIXELSIZE, EXT_RGB_RED,
    EXT_XBGR_BLUE, EXT_XBGR_GREEN, EXT_XBGR_PIXELSIZE, EXT_XBGR_RED, EXT_XRGB_BLUE, EXT_XRGB_GREEN,
...
/* 2:1 vertical sampling case: may need a spare row. */
{
    ...
    if (*upsample).spare_full != 0 {
        ...
        let mut work_ptrs: [JSAMPROW; 2] = [::std::ptr::null_mut::<JSAMPLE>(); 2];
        num_rows = 2u32;
        ...
        if num_rows > 1u32 {
            work_ptrs[1] = *output_buf.offset((*out_row_ctr + 1u32) as isize)
        } else {
...
pub unsafe extern "C" fn jinit_merged_upsampler(mut cinfo: j_decompress_ptr) {
    let mut upsample: my_upsample_ptr = Some(
        (*(*cinfo).mem)
            .alloc_small
            .expect("non-null function pointer"),
    )
    ...
    (*upsample).out_row_width = (*cinfo).output_width * (*cinfo).out_color_components as c_uint;
    if (*cinfo).max_v_samp_factor == 2i32 {
        ...
        .expect("non-null function pointer")(
            cinfo as j_common_ptr,
            JPOOL_IMAGE,
            (*upsample).out_row_width as c_ulong * ::std::mem::size_of::<JSAMPLE>() as c_ulong,
        ) as JSAMPROW
    } else {
        (*upsample).pub_0.upsample = Some(
            merged_1v_upsample
