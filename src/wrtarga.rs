...
/* used for unquantized full-color output */
{
    ...
    let mut outptr: *mut c_char = (*dest).iobuffer;
...
unsafe extern "C" fn put_demapped_gray(
    ...
    while col > 0u32 {
        ...
        let fresh3 = outptr;
        outptr = outptr.offset(1);
        *fresh3 = *color_map0.offset(*fresh2 as c_int as isize) as c_char;
        col -= 1
    }
    fwrite(
        (*dest).iobuffer as *const c_void,
...
unsafe extern "C" fn calc_buffer_dimensions_tga(
    ...
    let mut dest: tga_dest_ptr = dinfo as tga_dest_ptr;
    (*dest).buffer_width = (*cinfo).output_width * (*cinfo).output_components as c_uint;
}
/*
...
pub unsafe extern "C" fn jinit_write_targa(
    ...
    let mut dest: tga_dest_ptr = Some(
        (*(*cinfo).mem)
            .alloc_small
            .expect("non-null function pointer"),
    )
    ...
    .expect("non-null function pointer")(
        cinfo as j_common_ptr,
        JPOOL_IMAGE,
        (*dest).buffer_width as c_ulong * ::std::mem::size_of::<c_char>() as c_ulong,
    ) as *mut c_char;
