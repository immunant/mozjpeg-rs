...
unsafe extern "C" fn write_header(
    ...
    if num_colors > 0i32 {
        targaheader[1usize] = 1i32 as c_char;
        targaheader[5usize] = (num_colors & 0xffi32) as c_char;
        targaheader[6usize] = (num_colors >> 8i32) as c_char;
        targaheader[7usize] = 24i32 as c_char
    }
    targaheader[12usize] = ((*cinfo).output_width & 0xffi32 as c_uint) as c_char;
    targaheader[13usize] = ((*cinfo).output_width >> 8i32) as c_char;
        ...
        == JCS_GRAYSCALE as c_int as c_uint
    {
        ...
        targaheader[2usize] = 1i32 as c_char;
        targaheader[16usize] = 8i32 as c_char
    } else {
        targaheader[2usize] = 2i32 as c_char;
        targaheader[16usize] = 24i32 as c_char
    }
    if fwrite(
...
unsafe extern "C" fn put_pixel_rows(
    ...
    while col > 0i32 as c_uint {
        *outptr.offset(0isize) = *inptr.offset(2isize) as c_int as c_char;
        *outptr.offset(1isize) = *inptr.offset(1isize) as c_int as c_char;
        *outptr.offset(2isize) = *inptr.offset(0isize) as c_int as c_char;
        inptr = inptr.offset(3isize);
...
unsafe extern "C" fn start_output_tga(
        ...
        == JCS_RGB as c_int as c_uint
    {
        if 0 != (*cinfo).quantize_colors {
            ...
            while i < num_colors {
                putc(
                    *(*(*cinfo).colormap.offset(2isize)).offset(i as isize) as c_int,
                    outfile,
                );
                putc(
                    *(*(*cinfo).colormap.offset(1isize)).offset(i as isize) as c_int,
                    outfile,
                );
                putc(
                    *(*(*cinfo).colormap.offset(0isize)).offset(i as isize) as c_int,
                    outfile,
                );
                i += 1
            }
