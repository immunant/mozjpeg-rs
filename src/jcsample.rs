...
unsafe extern "C" fn int_downsample(
    ...
    let mut inrow: c_int = 0; /* outcol_h == outcol*h_expand */
...
unsafe extern "C" fn h2v2_smooth_downsample(
     ...
     */
    memberscale = (16384i32 - (*cinfo).smoothing_factor * 80i32) as JLONG; /* scaled (1-5*SF)/4 */
    neighscale = ((*cinfo).smoothing_factor * 16i32) as JLONG; /* scaled SF/4 */
    inrow = 0i32;
    outrow = 0i32;
    while outrow < (*compptr).v_samp_factor {
        outptr = *output_data.offset(outrow as isize);
...
unsafe extern "C" fn fullsize_smooth_downsample(
     ...
     * Also recall that SF = smoothing_factor / 1024.
     */
    memberscale = 65536i64 - (*cinfo).smoothing_factor as c_long * 512i64; /* scaled 1-8*SF */
    neighscale = ((*cinfo).smoothing_factor * 64i32) as JLONG; /* scaled SF */
    outrow = 0i32;
    while outrow < (*compptr).v_samp_factor {
        outptr = *output_data.offset(outrow as isize);
        inptr = *input_data.offset(outrow as isize);
