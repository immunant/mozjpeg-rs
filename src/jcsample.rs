...
unsafe extern "C" fn expand_right_edge(
    ...
    if numcols > 0i32 {
        ...
        while row < num_rows {
            let mut ptr: JSAMPROW = (*image_data.offset(row as isize)).offset(input_cols as isize);
            let mut pixval: JSAMPLE = *ptr.offset(-1isize);
            let mut count: c_int = numcols;
            while count > 0i32 {
                let fresh0 = ptr;
                ptr = ptr.offset(1);
                *fresh0 = pixval;
...
unsafe extern "C" fn sep_downsample(
    ...
    while ci < (*cinfo).num_components {
        let mut in_ptr: JSAMPARRAY = (*input_buf.offset(ci as isize)).offset(in_row_index as isize);
        let mut out_ptr: JSAMPARRAY = (*output_buf.offset(ci as isize))
            .offset((out_row_group_index * (*compptr).v_samp_factor as c_uint) as isize);
        Some(
...
unsafe extern "C" fn int_downsample(
    ...
    let mut output_cols: JDIMENSION = (*compptr).width_in_blocks * DCTSIZE as c_uint;

    let mut h_expand: c_int = (*cinfo).max_h_samp_factor / (*compptr).h_samp_factor;
    let mut v_expand: c_int = (*cinfo).max_v_samp_factor / (*compptr).v_samp_factor;
    let mut numpix: c_int = h_expand * v_expand;
    let mut numpix2: c_int = numpix / 2i32;
    ...
    let mut outrow: c_int = 0i32;
    while outrow < (*compptr).v_samp_factor {
        let mut outptr: JSAMPROW = *output_data.offset(outrow as isize);
        let mut outcol: JDIMENSION = 0u32;
        let mut outcol_h: JDIMENSION = 0u32;
        while outcol < output_cols {
            let mut outvalue: JLONG = 0i64;
            let mut v: c_int = 0i32;
            while v < v_expand {
                let mut inptr: JSAMPROW =
                    (*input_data.offset((inrow + v) as isize)).offset(outcol_h as isize);
...
unsafe extern "C" fn fullsize_downsample(
    ...
    expand_right_edge(
        ...
        (*cinfo).max_v_samp_factor,
        (*cinfo).image_width,
        (*compptr).width_in_blocks * DCTSIZE as c_uint,
    );
}
/*
 * Downsample pixel values of a single component.
...
unsafe extern "C" fn h2v1_downsample(
    ...
    while outrow < (*compptr).v_samp_factor {
        let mut outptr: JSAMPROW = *output_data.offset(outrow as isize);
        let mut inptr: JSAMPROW = *input_data.offset(outrow as isize);
        let mut bias: c_int = 0i32;
        let mut outcol: JDIMENSION = 0u32;
        while outcol < output_cols {
            let fresh3 = outptr;
            outptr = outptr.offset(1);
            *fresh3 = (*inptr as c_int + *inptr.offset(1) as c_int + bias >> 1i32) as JSAMPLE;
            bias ^= 1i32;
...
unsafe extern "C" fn h2v2_downsample(
    ...
    while outrow < (*compptr).v_samp_factor {
        let mut outptr: JSAMPROW = *output_data.offset(outrow as isize);
        let mut inptr0: JSAMPROW = *input_data.offset(inrow as isize);
        let mut inptr1: JSAMPROW = *input_data.offset((inrow + 1i32) as isize);
        let mut bias: c_int = 1i32;
        let mut outcol: JDIMENSION = 0u32;
        while outcol < output_cols {
            let fresh4 = outptr;
            outptr = outptr.offset(1);
...
unsafe extern "C" fn h2v2_smooth_downsample(
    ...
    let mut memberscale: JLONG = (16384i32 - (*cinfo).smoothing_factor * 80i32) as JLONG;
    let mut neighscale: JLONG = ((*cinfo).smoothing_factor * 16i32) as JLONG;
    let mut inrow: c_int = 0i32;
    let mut outrow: c_int = 0i32;
    while outrow < (*compptr).v_samp_factor {
        let mut outptr: JSAMPROW = *output_data.offset(outrow as isize);
        let mut inptr0: JSAMPROW = *input_data.offset(inrow as isize);
        let mut inptr1: JSAMPROW = *input_data.offset((inrow + 1i32) as isize);
        let mut above_ptr: JSAMPROW = *input_data.offset((inrow - 1i32) as isize);
        let mut below_ptr: JSAMPROW = *input_data.offset((inrow + 2i32) as isize);
        let mut membersum: JLONG = (*inptr0 as c_int
            + *inptr0.offset(1) as c_int
            + *inptr1 as c_int
            + *inptr1.offset(1) as c_int) as JLONG;
        let mut neighsum: JLONG = (*above_ptr as c_int
...
unsafe extern "C" fn fullsize_smooth_downsample(
    ...
    /* scaled 1-8*SF */
    /* scaled SF */
    let mut memberscale: JLONG = 65536i64 - (*cinfo).smoothing_factor as c_long * 512i64;
    let mut neighscale: JLONG = ((*cinfo).smoothing_factor * 64i32) as JLONG;
    let mut outrow: c_int = 0i32;
    while outrow < (*compptr).v_samp_factor {
        let mut outptr: JSAMPROW = *output_data.offset(outrow as isize);
        let mut inptr: JSAMPROW = *input_data.offset(outrow as isize);
        let mut above_ptr: JSAMPROW = *input_data.offset((outrow - 1i32) as isize);
        let mut below_ptr: JSAMPROW = *input_data.offset((outrow + 1i32) as isize);
        /* Special case for first column */
        let fresh7 = above_ptr;
        above_ptr = above_ptr.offset(1);
        let fresh8 = below_ptr;
        below_ptr = below_ptr.offset(1);
        let mut colsum: c_int = *fresh7 as c_int + *fresh8 as c_int + *inptr as c_int;
        let fresh9 = inptr;
        inptr = inptr.offset(1);

        let mut membersum: JLONG = *fresh9 as c_int as JLONG;
        let mut nextcolsum: c_int = *above_ptr as c_int + *below_ptr as c_int + *inptr as c_int;
        let mut neighsum: JLONG =
            colsum as c_long + (colsum as c_long - membersum) + nextcolsum as c_long;
        membersum = membersum * memberscale + neighsum * neighscale;
        let fresh10 = outptr;
        outptr = outptr.offset(1);
        *fresh10 = (membersum + 32768i64 >> 16i32) as JSAMPLE;
        ...
        while colctr > 0u32 {
            ...
            let fresh12 = outptr;
            outptr = outptr.offset(1);
            *fresh12 = (membersum + 32768i64 >> 16i32) as JSAMPLE;
            lastcolsum = colsum;
            colsum = nextcolsum;
            colctr -= 1
        }
...
pub unsafe extern "C" fn jinit_downsampler(mut cinfo: j_compress_ptr) {
    ...
    let mut downsample: my_downsample_ptr = Some(
        (*(*cinfo).mem)
            .alloc_small
            .expect("non-null function pointer"),
    )
