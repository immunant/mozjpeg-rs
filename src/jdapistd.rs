...
use libc::{c_uint, intptr_t, c_int, c_long, self};pub use crate::jmorecfg_h::{boolean, FALSE, JCOEF, JDIMENSION, JOCTET,
                            JSAMPLE, TRUE, UINT16, UINT8};pub use crate::jpegint_h::{inverse_DCT_method_ptr, jdiv_round_up,
                           jinit_master_decompress, jinit_upsampler,
                           DSTATE_BUFIMAGE, DSTATE_BUFPOST, DSTATE_PRELOAD,
                           DSTATE_PRESCAN, DSTATE_RAW_OK, DSTATE_READY,
                           DSTATE_SCANNING, JBUF_CRANK_DEST, JBUF_PASS_THRU,
                           ...
                           JPEG_REACHED_SOS, JPEG_ROW_COMPLETED,
                           JPEG_SUSPENDED, JQUANT_TBL, JSAMPARRAY, JSAMPIMAGE,
                           JSAMPROW, J_COLOR_SPACE, J_DCT_METHOD,
                           J_DITHER_MODE};pub use super::jdmainct::{my_main_controller, my_main_ptr,
                          set_wraparound_pointers, CTX_PREPARE_FOR_IMCU};pub use super::jdcoefct::{my_coef_controller, my_coef_ptr, start_iMCU_row};pub use super::jdsample::{my_upsample_ptr, my_upsampler, upsample1_ptr};pub use super::jerror::{C2RustUnnamed_3, JERR_ARITH_NOTIMPL,
    ...
    while (*(*cinfo).master).is_dummy_pass != 0 {
        ...
        while (*cinfo).output_scanline < (*cinfo).output_height {
            ...
            if !(*cinfo).progress.is_null() {
             ...
             let mut last_scanline:   JDIMENSION =
...
pub unsafe extern "C" fn jpeg_crop_scanline(
    ...
    *width = *width + input_xoffset - *xoffset;
    ...
    while ci < (*cinfo).num_components {
         ...
         let mut orig_downsampled_width:   c_int =
     (*compptr).downsampled_width as c_int;
        (*compptr).downsampled_width = jdiv_round_up(
            (
            (*cinfo)
                .output_width * (*compptr).h_samp_factor as c_uint) as c_long,
        ...
        (*(*cinfo).master).first_MCU_col[ci as usize] =
            ((((*xoffset * hsf as c_uint))) as c_long
                / align as c_long) as JDIMENSION;
...
unsafe extern "C" fn increment_simple_rowgroup_ctr(
    ...
    let mut main_ptr: super::jdmainct::my_main_ptr =
        (*cinfo).main as super::jdmainct::my_main_ptr;
    /* Increment the counter to the next row group after the skipped rows. */
    (*main_ptr).rowgroup_ctr = (*main_ptr).rowgroup_ctr +
    rows / (*cinfo).max_v_samp_factor as c_uint;
...
pub unsafe extern "C" fn jpeg_skip_scanlines(
    ...
    mut num_lines: JDIMENSION,
         ...
         let mut lines_to_skip:  JDIMENSION =  0; let mut main_ptr: super::jdmainct::my_main_ptr =
        (*cinfo).main as super::jdmainct::my_main_ptr;
    let mut coef: super::jdcoefct::my_coef_ptr =
        (*cinfo).coef as super::jdcoefct::my_coef_ptr;
    ...
    if num_lines == 0u32 {
     ...
     ((*cinfo).min_DCT_scaled_size * (*cinfo).max_v_samp_factor)
        as JDIMENSION; let mut lines_left_in_iMCU_row:   JDIMENSION =
     ( lines_per_iMCU_row - (*cinfo).output_scanline % lines_per_iMCU_row) %
    lines_per_iMCU_row; let mut lines_after_iMCU_row:   JDIMENSION =
     ...
     * have to.
     */
     let mut lines_to_read:   JDIMENSION =
