...
unsafe extern "C" fn post_process_1pass(
    ...
    let mut max_rows: JDIMENSION = out_rows_avail - *out_row_ctr;
...
unsafe extern "C" fn post_process_prepass(
    ...
    if (*post).next_row > old_next_row {
        let mut num_rows: JDIMENSION = (*post).next_row - old_next_row;
        Some(
            (*(*cinfo).cquantize)
                .color_quantize
                .expect("non-null function pointer"),
    ...
    if (*post).next_row >= (*post).strip_height {
        (*post).starting_row = (*post).starting_row + (*post).strip_height;
        (*post).next_row = 0u32
    };
}
/*
...
unsafe extern "C" fn post_process_2pass(
    ...
    /* available in strip */
    let mut num_rows: JDIMENSION = (*post).strip_height - (*post).next_row;
    let mut max_rows: JDIMENSION = out_rows_avail - *out_row_ctr; /* available in output area */
    ...
    if (*cinfo).quantize_colors != 0 {
        ...
        if need_full_buffer != 0 {
            ...
            .expect("non-null function pointer")(
                ...
                JPOOL_IMAGE,
                FALSE,
                (*cinfo).output_width * (*cinfo).out_color_components as c_uint,
                jround_up(
