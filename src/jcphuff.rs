pub use super::jchuff::{
    c_derived_tbl, jpeg_gen_optimal_table, jpeg_make_c_derived_tbl, MAX_COEF_BITS,
};
...
unsafe extern "C" fn start_pass_phuff(mut cinfo: j_compress_ptr, mut gather_statistics: boolean) {
    ...
    let mut is_DC_band: boolean = ((*cinfo).Ss == 0i32) as c_int;
    ...
    if (*cinfo).Ah == 0i32 {
        ...
        if (*entropy).bit_buffer.is_null() {
            ...
            .expect("non-null function pointer")(
                ...
                JPOOL_IMAGE,
                MAX_CORR_BITS as c_ulong * ::std::mem::size_of::<c_char>() as c_ulong,
            ) as *mut c_char
        }
    }
...
/* Emit some bits, unless we are in gather mode */
{
    ...
    if size == 0i32 {
        (*(*(*entropy).cinfo).err).msg_code = super::jerror::JERR_HUFF_MISSING_CODE as c_int; /* do nothing if we're only getting stats */
        Some(
            (*(*(*entropy).cinfo).err)
                .error_exit
                .expect("non-null function pointer"),
...
unsafe extern "C" fn emit_symbol(
    ...
    if (*entropy).gather_statistics != 0 {
        ...
        *fresh2 += 1
        ...
        let mut tbl: *mut super::jchuff::c_derived_tbl = (*entropy).derived_tbls[tbl_no as usize];
        emit_bits(
            entropy,
...
unsafe extern "C" fn emit_eobrun(mut entropy: phuff_entropy_ptr) {
    if (*entropy).EOBRUN > 0u32 {
        let mut temp: c_int = (*entropy).EOBRUN as c_int;
        let mut nbits: c_int = jpeg_nbits_table[temp as usize] as c_int - 1i32;
        /* safety check: shouldn't happen given limited correction-bit buffer */
        if nbits > 14i32 {
            (*(*(*entropy).cinfo).err).msg_code = super::jerror::JERR_HUFF_MISSING_CODE as c_int;
            Some(
                (*(*(*entropy).cinfo).err)
                    .error_exit
                    .expect("non-null function pointer"),
...
unsafe extern "C" fn encode_mcu_DC_first(
    ...
    while blkn < (*cinfo).blocks_in_MCU {
        ...
        let mut compptr: *mut jpeg_component_info = (*cinfo).cur_comp_info[ci as usize];
        let mut temp2: c_int = (*block)[0] as c_int >> Al;
        let mut temp: c_int = temp2 - (*entropy).last_dc_val[ci as usize];
         ...
         */
        let mut temp3: c_int =
            temp >> CHAR_BIT as c_ulong * ::std::mem::size_of::<c_int>() as c_ulong - 1u64; /* temp is abs value of input */
        temp ^= temp3;
        temp -= temp3;
...
unsafe extern "C" fn encode_mcu_AC_first(
    ...
    let mut values: *mut JCOEF = (values_unaligned.as_mut_ptr() as size_t + 16u64 - 1u64
        & !(16i32 - 1i32) as c_ulong) as *mut JCOEF;
    let mut cvalue: *const JCOEF = values;
    /* Prepare data */
    (*entropy)
        .AC_first_prepare
        .expect("non-null function pointer")(
    ...
    while zerobits != 0 {
        ...
        let mut temp: c_int = *cvalue.offset(0) as c_int;
        let mut temp2: c_int = *cvalue.offset(DCTSIZE2 as isize) as c_int;
        while r > 15i32 {
...
unsafe extern "C" fn encode_mcu_DC_refine(
    ...
    while blkn < (*cinfo).blocks_in_MCU {
        let mut block: JBLOCKROW = *MCU_data.offset(blkn as isize);
        let mut temp: c_int = (*block)[0] as c_int;
...
unsafe extern "C" fn encode_mcu_AC_refine(
    ...
    let mut absvalues: *mut JCOEF = (absvalues_unaligned.as_mut_ptr() as size_t + 16u64 - 1u64
        & !(16i32 - 1i32) as c_ulong) as *mut JCOEF;
    let mut cabsvalue: *const JCOEF = absvalues;
    let mut EOBPTR: *const JCOEF = absvalues.offset((*entropy)
        .AC_refine_prepare
        .expect("non-null function pointer")(
        (*(*MCU_data.offset(0)).offset(0)).as_mut_ptr(),
        jpeg_natural_order.as_ptr().offset((*cinfo).Ss as isize),
    ...
    let mut BR: c_uint = 0u32;
    let mut BR_buffer: *mut c_char = (*entropy).bit_buffer.offset((*entropy).BE as isize);
    let mut zerobits: size_t = bits[0];
    let mut signbits: size_t = bits[1];
    while zerobits != 0 {
...
pub unsafe extern "C" fn jinit_phuff_encoder(mut cinfo: j_compress_ptr) {
    let mut entropy: phuff_entropy_ptr = Some(
        (*(*cinfo).mem)
            .alloc_small
            .expect("non-null function pointer"),
    )
    ...
    while i < NUM_HUFF_TBLS {
        (*entropy).derived_tbls[i as usize] = NULL as *mut super::jchuff::c_derived_tbl;
