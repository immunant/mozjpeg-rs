...
unsafe extern "C" fn copy_pixel_rows(
    ...
    let mut ptr: JSAMPROW = *(*dest).pub_0.buffer.offset(0);
    let mut bufferptr: *mut c_char = (*dest).iobuffer;
    memcpy(
...
unsafe extern "C" fn put_rgb(
    ...
    let mut ptr: JSAMPROW = *(*dest).pub_0.buffer.offset(0);
    let mut bufferptr: *mut c_char = (*dest).iobuffer;
    let mut col: JDIMENSION = (*cinfo).output_width;
    while col > 0u32 {
...
unsafe extern "C" fn put_demapped_gray(
    ...
    while col > 0u32 {
        ...
        bufferptr = bufferptr.offset(1);
        *fresh15 = *color_map.offset(*fresh14 as c_int as isize) as c_char;
        col -= 1
    }
    fwrite(
        (*dest).iobuffer as *const c_void,
...
unsafe extern "C" fn calc_buffer_dimensions_ppm(
    ...
    if (*cinfo).out_color_space == JCS_GRAYSCALE {
        (*dest).samples_per_row = (*cinfo).output_width * (*cinfo).out_color_components as c_uint
    } else {
        (*dest).samples_per_row = (*cinfo).output_width * 3u32
    }
    (*dest).buffer_width = (*dest).samples_per_row as c_ulong
        * (BYTESPERSAMPLE as c_ulong * ::std::mem::size_of::<c_char>() as c_ulong);
}
/*
...
pub unsafe extern "C" fn jinit_write_ppm(
    ...
    let mut dest: ppm_dest_ptr = Some(
        (*(*cinfo).mem)
            .alloc_small
            .expect("non-null function pointer"),
    )
        ...
        || (*cinfo).out_color_space != JCS_EXT_RGB && (*cinfo).out_color_space != JCS_RGB
    {
        ...
        .expect("non-null function pointer")(
            cinfo as j_common_ptr,
            JPOOL_IMAGE,
            (*cinfo).output_width * (*cinfo).output_components as c_uint,
            1u32,
        );
