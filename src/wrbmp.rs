...
/* Write a Windows-style BMP file header, including colormap if needed */
{
    ...
    /* Header and colormap */
    let mut headersize: c_long = (14i32 + 40i32 + cmap_entries * 4i32) as c_long;
    let mut bfSize: c_long =
    ...
    bmpfileheader[(2i32 + 1i32) as usize] = (bfSize >> 8i32 & 0xffi64) as c_char;
    bmpfileheader[(2i32 + 2i32) as usize] = (bfSize >> 16i32 & 0xffi64) as c_char;
    bmpfileheader[(2i32 + 3i32) as usize] = (bfSize >> 24i32 & 0xffi64) as c_char;
    /* we leave bfReserved1 & bfReserved2 = 0 */
    bmpfileheader[10] = (headersize & 0xffi64) as c_char; /* bfOffBits */
    bmpfileheader[(10i32 + 1i32) as usize] = (headersize >> 8i32 & 0xffi64) as c_char;
    bmpfileheader[(10i32 + 2i32) as usize] = (headersize >> 16i32 & 0xffi64) as c_char;
    bmpfileheader[(10i32 + 3i32) as usize] = (headersize >> 24i32 & 0xffi64) as c_char;
    ...
    bmpinfoheader[(4i32 + 1i32) as usize] = ((*cinfo).output_width >> 8i32 & 0xffu32) as c_char; /* biPlanes - must be 1 */
    bmpinfoheader[(4i32 + 2i32) as usize] = ((*cinfo).output_width >> 16i32 & 0xffu32) as c_char; /* biBitCount */
    bmpinfoheader[(4i32 + 3i32) as usize] = ((*cinfo).output_width >> 24i32 & 0xffu32) as c_char;
    bmpinfoheader[8] = ((*cinfo).output_height & 0xffu32) as c_char;
    bmpinfoheader[(8i32 + 1i32) as usize] = ((*cinfo).output_height >> 8i32 & 0xffu32) as c_char;
    bmpinfoheader[(8i32 + 2i32) as usize] = ((*cinfo).output_height >> 16i32 & 0xffu32) as c_char;
    bmpinfoheader[(8i32 + 3i32) as usize] = ((*cinfo).output_height >> 24i32 & 0xffu32) as c_char;
    ...
    if (*cinfo).density_unit as c_int == 2i32 {
        ...
        bmpinfoheader[24] = (((*cinfo).X_density as c_int * 100i32) as c_long & 0xffi64) as c_char; /* XPels/M */
        bmpinfoheader[(24i32 + 1i32) as usize] =
            (((*cinfo).X_density as c_int * 100i32) as c_long >> 8i32 & 0xffi64) as c_char;
        bmpinfoheader[(24i32 + 2i32) as usize] =
            (((*cinfo).X_density as c_int * 100i32) as c_long >> 16i32 & 0xffi64) as c_char;
        bmpinfoheader[(24i32 + 3i32) as usize] =
            (((*cinfo).X_density as c_int * 100i32) as c_long >> 24i32 & 0xffi64) as c_char;
        bmpinfoheader[28] = (((*cinfo).Y_density as c_int * 100i32) as c_long & 0xffi64) as c_char;
        bmpinfoheader[(28i32 + 1i32) as usize] =
            (((*cinfo).Y_density as c_int * 100i32) as c_long >> 8i32 & 0xffi64) as c_char;
        bmpinfoheader[(28i32 + 2i32) as usize] =
            (((*cinfo).Y_density as c_int * 100i32) as c_long >> 16i32 & 0xffi64) as c_char;
        bmpinfoheader[(28i32 + 3i32) as usize] =
            (((*cinfo).Y_density as c_int * 100i32) as c_long >> 24i32 & 0xffi64) as c_char
        /* XPels/M */
    } /* biClrUsed */
...
/* Write an OS2-style BMP file header, including colormap if needed */
{
    ...
    /* File size */
    /* Header and colormap */
    let mut headersize: c_long = (14i32 + 12i32 + cmap_entries * 3i32) as c_long;
    let mut bfSize: c_long =
        headersize + (*dest).row_width as c_long * (*cinfo).output_height as c_long;
    /* Set unused fields of header to 0 */
    memset(
    ...
    bmpcoreheader[(4i32 + 1i32) as usize] = ((*cinfo).output_width >> 8i32 & 0xffu32) as c_char; /* bcPlanes - must be 1 */
    bmpcoreheader[6] = ((*cinfo).output_height & 0xffu32) as c_char; /* bcBitCount */
    bmpcoreheader[(6i32 + 1i32) as usize] = ((*cinfo).output_height >> 8i32 & 0xffu32) as c_char;
...
unsafe extern "C" fn finish_output_bmp(
    ...
    if (*dest).use_inversion_array != 0 {
        ...
        while row > 0u32 {
            if !progress.is_null() {
                (*progress).pub_0.pass_counter = ((*cinfo).output_height - row) as c_long;
            ...
            let mut data_ptr: JSAMPROW = *image_ptr.offset(0);
            let mut col: JDIMENSION = (*dest).row_width;
            while col > 0u32 {
...
pub unsafe extern "C" fn jinit_write_bmp(
    ...
    let mut dest: bmp_dest_ptr = Some(
        (*(*cinfo).mem)
            .alloc_small
            .expect("non-null function pointer"),
    )
    ...
    {
        row_width = (*cinfo).output_width * (*cinfo).output_components as c_uint;
        (*dest).data_width = (*cinfo).output_width * 3u32;
        (*dest).row_width = (*dest).data_width
    } else {
        row_width = (*cinfo).output_width * (*cinfo).output_components as c_uint;
    ...
    .expect("non-null function pointer")(
        cinfo as j_common_ptr, JPOOL_IMAGE, row_width, 1u32
    );
