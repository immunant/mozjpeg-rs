...
unsafe extern "C" fn pre_process_data(
    ...
    while *in_row_ctr < in_rows_avail && *out_row_group_ctr < out_row_groups_avail {
        let mut ci: c_int = 0;
        let mut inrows: JDIMENSION = in_rows_avail - *in_row_ctr;
        let mut numrows: c_int = (*cinfo).max_v_samp_factor - (*prep).next_buf_row;
        ...
        *in_row_ctr = *in_row_ctr + numrows as c_uint;
        ...
        if (*prep).rows_to_go == 0u32 && (*prep).next_buf_row < (*cinfo).max_v_samp_factor {
            ci = 0i32;
         ...
         */
        if !((*prep).rows_to_go == 0u32 && *out_row_group_ctr < out_row_groups_avail) {
            continue;
        }
        ci = 0i32;
        ...
        while ci < (*cinfo).num_components {
            expand_bottom_edge(
                *output_buf.offset(ci as isize),
                ...
                (*out_row_group_ctr * (*compptr).v_samp_factor as c_uint) as c_int,
                (out_row_groups_avail * (*compptr).v_samp_factor as c_uint) as c_int,
            );
            ci += 1;
...
unsafe extern "C" fn pre_process_context(
    ...
    while *out_row_group_ctr < out_row_groups_avail {
        ...
        if *in_row_ctr < in_rows_avail {
            let mut inrows: JDIMENSION = in_rows_avail - *in_row_ctr;
            let mut numrows: c_int = (*prep).next_buf_stop - (*prep).next_buf_row;
            numrows = if (numrows as JDIMENSION) < inrows {
                numrows as JDIMENSION
            } else {
                inrows
...
unsafe extern "C" fn create_context_buffer(mut cinfo: j_compress_ptr) {
    ...
    let mut fake_buffer: JSAMPARRAY = Some(
        (*(*cinfo).mem)
            .alloc_small
            .expect("non-null function pointer"),
    )
    ...
    while ci < (*cinfo).num_components {
         ...
         */
        let mut true_buffer: JSAMPARRAY = Some(
            (*(*cinfo).mem)
                .alloc_sarray
                .expect("non-null function pointer"),
        )
        ...
        memcpy(
            ...
            (3i32 * rgroup_height) as c_ulong * ::std::mem::size_of::<JSAMPROW>() as c_ulong,
        );
