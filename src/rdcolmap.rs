...
pub use crate::jmorecfg_h::{
    boolean, JCOEF, JDIMENSION, JOCTET, JSAMPLE, MAXJSAMPLE, UINT16, UINT8,
};
pub use crate::jpeglib_h::{
    ...
    JPOOL_IMAGE, JQUANT_TBL, JSAMPARRAY, JSAMPROW, J_COLOR_SPACE, J_DCT_METHOD, J_DITHER_MODE,
};
pub use crate::stddef_h::size_t;
pub use crate::stdlib::{
    _IO_codecvt, _IO_lock_t, _IO_marker, _IO_wide_data, __off64_t, __off_t, getc, EOF, FILE,
    _IO_FILE,
};
...
unsafe extern "C" fn read_gif_map(mut cinfo: j_decompress_ptr, mut infile: *mut FILE) {
    ...
    i = 0i32;
    while i < colormaplen {
        let mut R: c_int = getc(infile);
        let mut G: c_int = getc(infile);
        let mut B: c_int = getc(infile);
...
unsafe extern "C" fn read_ppm_map(mut cinfo: j_decompress_ptr, mut infile: *mut FILE) {
    ...
    let mut w: c_uint = read_pbm_integer(cinfo, infile);
    let mut h: c_uint = read_pbm_integer(cinfo, infile);
    let mut maxval: c_uint = read_pbm_integer(cinfo, infile);
