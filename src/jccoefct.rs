    ...
    while yoffset < (*coef).MCU_rows_per_iMCU_row {
        ...
        while MCU_col_num <= last_MCU_col {
            ...
            while ci < (*cinfo).comps_in_scan {
                ...
                let mut xpos: JDIMENSION = MCU_col_num * (*compptr).MCU_sample_width as c_uint;
                let mut ypos: JDIMENSION = (yoffset * DCTSIZE) as JDIMENSION;
...
unsafe extern "C" fn compress_first_pass(
    ...
    while ci < (*cinfo).num_components {
        ...
        let mut thisblockrow: JBLOCKROW = ::std::ptr::null_mut::<JBLOCK>();
        ...
        let mut buffer_dst: JBLOCKARRAY = Some(
            (*(*cinfo).mem)
                .access_virt_barray
                .expect("non-null function pointer"),
        )
        ...
        let mut blocks_across: JDIMENSION = (*compptr).width_in_blocks;
        let mut h_samp_factor: c_int = (*compptr).h_samp_factor;
        let mut ndummy: c_int = (blocks_across % h_samp_factor as c_uint) as c_int;
        if ndummy > 0i32 {
        ...
        while block_row < block_rows {
            ...
            if ndummy > 0i32 {
                ...
                jzero_far(
                    ...
                    ndummy as c_ulong * ::std::mem::size_of::<JBLOCK>() as c_ulong,
                );
        ...
        if (*coef).iMCU_row_num == last_iMCU_row {
            ...
            let mut MCUs_across: JDIMENSION = blocks_across / h_samp_factor as c_uint; /* advance to next MCU in row */
            block_row = block_rows;
            while block_row < (*compptr).v_samp_factor {
                thisblockrow = *buffer.offset(block_row as isize);
                let mut lastblockrow: JBLOCKROW = *buffer.offset((block_row - 1i32) as isize);
                jzero_far(
                    thisblockrow as *mut c_void,
                    blocks_across as c_ulong * ::std::mem::size_of::<JBLOCK>() as c_ulong,
                );
...
unsafe extern "C" fn compress_trellis_pass(
    ...
    while ci < (*cinfo).comps_in_scan {
        ...
        let mut block_rows: c_int = 0;
        let mut thisblockrow: JBLOCKROW = ::std::ptr::null_mut::<JBLOCK>();
        let mut lastblockrow: JBLOCKROW = ::std::ptr::null_mut::<JBLOCK>();
