...
use crate::jsimd::jsimd_can_h2v1_fancy_upsample;
use crate::jsimd::jsimd_can_h2v1_upsample;
use crate::jsimd::jsimd_can_h2v2_fancy_upsample;
use crate::jsimd::jsimd_can_h2v2_upsample;
use crate::jsimd::jsimd_h2v1_fancy_upsample;
use crate::jsimd::jsimd_h2v1_upsample;
use crate::jsimd::jsimd_h2v2_fancy_upsample;
use crate::jsimd::jsimd_h2v2_upsample;
...
unsafe extern "C" fn sep_upsample(
    ...
    if (*upsample).next_row_out >= (*cinfo).max_v_samp_factor {
        ...
        while ci < (*cinfo).num_components {
            (*upsample).methods[ci as usize].expect("non-null function pointer")(
                cinfo,
                compptr,
                (*input_buf.offset(ci as isize)).offset(
                    (*in_row_group_ctr)
                        .wrapping_mul((*upsample).rowgroup_height[ci as usize] as c_uint)
                        as isize,
                ),
...
unsafe extern "C" fn h2v2_fancy_upsample(
    ...
    while outrow < (*cinfo).max_v_samp_factor {
        ...
        while v < 2i32 {
            ...
            let fresh22 = inptr1;
            inptr1 = inptr1.offset(1);
            thiscolsum = *fresh21 as c_int * 3i32 + *fresh22 as c_int;
            let fresh23 = inptr0;
            inptr0 = inptr0.offset(1);
            let fresh24 = inptr1;
            inptr1 = inptr1.offset(1);
            nextcolsum = *fresh23 as c_int * 3i32 + *fresh24 as c_int;
            let fresh25 = outptr;
            outptr = outptr.offset(1);
...
pub unsafe extern "C" fn jinit_upsampler(mut cinfo: j_decompress_ptr) {
    ...
    let mut v_in_group: c_int = 0;
    let mut h_out_group: c_int = 0;
    let mut v_out_group: c_int = 0;
    if 0 == (*(*cinfo).master).jinit_upsampler_no_alloc {
        upsample = (*(*cinfo).mem)
            .alloc_small
            .expect("non-null function pointer")(
    ...
    while ci < (*cinfo).num_components {
        ...
        if 0 == (*compptr).component_needed {
            ...
            if 0 != do_fancy && (*compptr).downsampled_width > 2i32 as c_uint {
                if 0 != jsimd_can_h2v1_fancy_upsample() {
                    (*upsample).methods[ci as usize] = Some(
            ...
            } else if 0 != jsimd_can_h2v1_upsample() {
                (*upsample).methods[ci as usize] = Some(
                    jsimd_h2v1_upsample
                        as unsafe extern "C" fn(
                            _: j_decompress_ptr,
        ...
        } else if h_in_group == h_out_group && v_in_group * 2i32 == v_out_group && 0 != do_fancy {
            (*upsample).methods[ci as usize] = Some(
                h1v2_fancy_upsample
                    as unsafe extern "C" fn(
                        _: j_decompress_ptr,
            ...
            if 0 != do_fancy && (*compptr).downsampled_width > 2i32 as c_uint {
                if 0 != jsimd_can_h2v2_fancy_upsample() {
                    (*upsample).methods[ci as usize] = Some(
                ...
                (*upsample).pub_0.need_context_rows = TRUE
            } else if 0 != jsimd_can_h2v2_upsample() {
                (*upsample).methods[ci as usize] = Some(
                    jsimd_h2v2_upsample
                        as unsafe extern "C" fn(
                            _: j_decompress_ptr,
