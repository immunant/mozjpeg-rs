...
/* A comment/newline sequence is returned as a newline */ {
     ...
     let mut ch:   libc::c_int =  crate::stdlib::getc(file);
    if ch == '#' as i32 {
        loop {
...
/* Reads one trailing character after the integer; returns it in termchar */ {
    ...
    /* Skip any leading whitespace, detect EOF */
    {
        ...
        if !(*(*crate::stdlib::__ctype_b_loc()).offset(ch as isize) as libc::c_int
            &  crate::stdlib::_ISspace as libc::c_ushort as libc::c_int
            != 0)
        {
            break;
        }
        ...
        == 0
    {
     ...
     let mut val:   libc::c_long =  (ch - '0' as i32) as libc::c_long;
    loop {
        ch = text_getc(file);
        if !(ch != crate::stdlib::EOF) {
            break;
 ...
 */ {
       ...
       let mut termchar:  libc::c_int =  0; let mut val:  libc::c_long =  0;
     let mut fp:   *mut crate::stdlib::FILE =
     crate::stdlib::fopen(filename,  b"r\x00".as_ptr() as *const libc::c_char);
    if fp.is_null() {
    ...
    while read_text_integer(fp, &mut val, &mut termchar) != 0 {
         let mut table:  [libc::c_uint; 64] =  [0; 64];if tblno >= crate::jpeglib_h::NUM_QUANT_TBLS {
 ...
 */ {
    ...
    if read_text_integer(file, result, termchar) == 0 {
     ...
     let mut ch:   libc::c_int =  *termchar;
 ...
 */ {
       ...
       let mut termchar:  libc::c_int =  0; let mut val:  libc::c_long =  0;  let mut scans:  [crate::jpeglib_h::jpeg_scan_info; 100] =
    ...
    std::ffi::CStr::from_ptr(filename as
     ...
     let mut scanptr:   *mut crate::jpeglib_h::jpeg_scan_info =  scans.as_mut_ptr(); let mut scanno:   libc::c_int =  0i32;
    ...
    std::ffi::CStr::from_ptr(filename as
        ...
        (*scanptr).component_index[0] = val as libc::c_int;
         let mut ncomps:   libc::c_int =  1i32;
        loop {
            if !(termchar == ' ' as i32) {
 ...
 */ {
     ...
     let mut val:  libc::c_float =  75.0f32; 
     let mut tblno:   libc::c_int =  0i32;
    while tblno < crate::jpeglib_h::NUM_QUANT_TBLS {
        if *arg != 0 {
              let mut ch:   libc::c_char =   ',' as libc::c_char;
    ...
    if val >= 90f32 {
        set_sample_factors(
            ...
            b"1x1\x00".as_ptr() as *mut libc::c_char,
        );
    } else if val >= 80f32 {
        set_sample_factors(
            cinfo,
            ...
            b"2x1\x00".as_ptr() as *mut libc::c_char,
        );
    }
    return crate::jmorecfg_h::TRUE;
}
 ...
 */ {
    ...
    while ci < crate::jmorecfg_h::MAX_COMPONENTS {
        if *arg != 0 {
             let mut val1:  libc::c_int =  0; let mut val2:  libc::c_int =  0; let mut ch1:  libc::c_char =  0;  let mut ch2:   libc::c_char =   ',' as libc::c_char;
            if crate::stdlib::sscanf(
                arg,
